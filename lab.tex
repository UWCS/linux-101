...
What do you see? 
For the moment, there should just be a few folders, also referred to as sub-directories. If you want to see some more details, use \texttt{ls -l} 
...
We use this argument to describe where we would like to change directory to.
...
\begin{tabular}{l|c}
    ...
    \Large \texttt{cd ..} \normalfont & Change to the parent directory (a level above your current one). \\
...
Moving up a directory (\texttt{cd ..}) then to \texttt{cd scripts} could  be combined into \texttt{cd ../scripts}. These can be chained as deeply as you need.
...
If we want to be able to remove folders, we can override the default behaviour using \texttt{ rm -r}: the recursive flag:
...
You might think to create both folders using \texttt{mkdir public\_html/css}, but this will fail, as \texttt{mkdir} requires all the path up to the penultimate part existing already.
...
For this lab, we'll be using VS Code as a text editor.

\bfseries Task 4: \normalfont 
Using the \texttt{code <path>} command to open a file/folder, open up the \texttt{index.html} file we created and insert some HTML. 
...
Have a go sprucing up your website. Feel free to add some HTML or CSS if you like ...or ask the people around you to have a look at their websites?
